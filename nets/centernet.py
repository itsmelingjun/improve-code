...
from nets.hourglass import stardist, stardist_Proposal, FCN, stardist_Head
from nets.resnet50 import resnet50, resnet50_Decoder, resnet50_Head
from nets.stardist import *
...
class CenterNet_stardist(nn.Module):
    def __init__(self, num_classes=20, pretrained=False):
        super(CenterNet_stardist, self).__init__()
        self.pretrained = pretrained
        self.backbone = FCN(in_channels=3, out_channels=64)
        self.stardist_Proposal = stardist_Proposal(feature_map_size=(1368, 912))
        self.head = stardist_Head(channel=64, num_classes=num_classes)
    ...
    def forward(self, x):
        fp = self.backbone(x)
        return self.head(self.stardist_Proposal(fp, x))