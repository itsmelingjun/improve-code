...
from utils.utils import (canny_image, cvtColor, get_classes, preprocess_input, resize_image,
                         show_config)
...
class CenterNet(object):
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        #     label_width = label_bbox[2] - label_bbox[0]  # 计算文本宽度
        #     label_height = label_bbox[3] - label_bbox[1]  # 计算文本高度
        #
        #     # 将标签编码为UTF-8
        ...
        #
        #     # 绘制文本背景矩形
        #     draw.rectangle([tuple(text_origin), tuple(text_origin + np.array([label_width, label_height]))],
        #                    fill=self.colors[c])
        #
        #     # 绘制文本
    ...
    def draw_image(self, image,  results, crop=False, count=False):
        # ---------------------------------------------------#
        #   计算输入图片的高和宽
        # ---------------------------------------------------#
        image_shape = np.array(np.shape(image)[0:2])
    ...
    def get_FPS(self, image, test_interval):
        ...
        for _ in range(test_interval):
            with torch.no_grad():
                ...
                #   利用预测结果进行解码
                #-----------------------------------------------------------#
                #outputs[0] = gaussian_blur2d(outputs[0], (7, 7), sigma=(3, 3))#删
                outputs = decode_bbox(outputs[0], outputs[1], outputs[2], self.confidence, self.cuda)
    ...
    def detect_heatmap(self, image, heatmap_save_path):
        ...
        #
        #     # 提取热图并进行处理
        #     heatmap = outputs[0][0].permute(1, 2, 0).cpu().numpy()  # 转换为 NumPy 数组
        #     heatmap = cv2.GaussianBlur(heatmap, (5, 5), sigmaX=1.5, sigmaY=1.5)  # 应用高斯滤波
        #
        #     # 确保使用正确的图像尺寸
        #     img_height, img_width = image.shape[:2] if isinstance(image, np.ndarray) else image.size
        #
        #     plt.clf()  # 清除当前图形
        ...
        #
        #     # 计算分数
        #     score = np.max(heatmap, axis=-1)  # 在最后一维计算最大值
        #     score = cv2.resize(score, (img_width, img_height), interpolation=cv2.INTER_LINEAR)  # 调整大小
        #     normed_score = (score * 255).astype('uint8')  # 归一化到[0, 255]
        #     mask = np.maximum(mask, normed_score)  # 生成掩膜
        #
        #     # 可视化热图
